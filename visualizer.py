...
# Rider density
im_riders = axs[0].imshow(
    sim.rider_grid,
    cmap="Reds",
    vmin=0,
    vmax=MAX_RIDERS_PER_CELL,
    animated=True
)
axs[0].set_title("Rider Density")
plt.colorbar(im_riders, ax=axs[0], fraction=0.046)

# Driver density
im_drivers = axs[1].imshow(
    sim.driver_grid,
    cmap="Blues",
    vmin=0,
    vmax=MAX_DRIVERS_PER_CELL,
    animated=True
)
axs[1].set_title("Driver Density")
plt.colorbar(im_drivers, ax=axs[1], fraction=0.046)
...
def update(frame):
    for _ in range(5):
        if sim.time < SIM_MINUTES:
            sim.step()

    im_riders.set_array(sim.rider_grid)
    im_drivers.set_array(sim.driver_grid)

    fig.suptitle(f"Minute {sim.time}", fontsize=14)
    return im_riders, im_drivers
