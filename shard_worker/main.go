...
// ------------------------- Redis helpers -------------------------
func storeRiderMapping(rdb *redis.Client, extID, shardID, seqNo int, lat, lon float64) {
    ...
    // expire after 24h to clean up stale entries
...
func handleAddRider(rdb *redis.Client, shardID int) gin.HandlerFunc {
    return func(c *gin.Context) {
        ...
        // Store mapping immediately (seq_no 0 for now; will be updated later by event consumer)
        storeRiderMapping(rdb, req.ExtID, shardID, 0, req.Lat, req.Lon)
...
func startEventConsumer(rdb *redis.Client, shardID int) {
    ...
    for {
        ...
        case "RIDER_ADDED":
            ...
            // Update mapping with correct seq_no
            storeRiderMapping(rdb, int(extID), int(shard), int(seqNo), lat, lon)
            // Add to geo set
